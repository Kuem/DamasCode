...
public class AlfaBetaIA extends AbstractIA {
	...
	private double valorMax(Nodo nodo, int profundidade, double a, double b) {
		if (isInterrompido()) {
			return 0;
		}
		...
		if (nodo.isTerminal()) {
			...
			for (Jogada jogada : nodo.getJogadasPermitidas()) {
				...
				utilidade = Math.max(utilidade,
						valorMin(sucessor, proximaProfundidade, a, b));
	...
	private double valorMin(Nodo nodo, int profundidade, double a, double b) {
		if (isInterrompido()) {
			return 0;
		}
		...
		if (nodo.isTerminal()) {
			...
			for (Jogada jogada : nodo.getJogadasPermitidas()) {
				...
				utilidade = Math.min(utilidade,
						valorMax(sucessor, proximaProfundidade, a, b));
